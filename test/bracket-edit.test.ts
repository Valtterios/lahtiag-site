...
describe('draft, seeding and going live', () => {
  ...
  it('draws a draft, takes a new seeding, then goes live once', async () => {
    ...
    expect((await getBracketRow(db(), bracket))?.live_at).toBeNull();
    const before = (await getBracket(db(), bracket)).filter((m) => m.round === 1);
    // Swap the two matches' first sides.
    await setBracketSeeding(db(), bracket, [
    ...
    expect(await goLiveBracket(db(), bracket, NOW + 10)).toBe(true);
    expect((await getBracketRow(db(), bracket))?.live_at).toBe(NOW + 10);
    expect(await goLiveBracket(db(), bracket, NOW + 20)).toBe(false);
    await expect(setBracketSeeding(db(), bracket, [[before[0].side_a, before[0].side_b], [before[1].side_a, before[1].side_b]])).rejects.toMatchObject({ code: 'bracket_live' });
    // A redraw is a draft again.
    await generateBracket(db(), bracket, NOW + 30);
    expect((await getBracketRow(db(), bracket))?.live_at).toBeNull();
  });
  ...
  it('keeps byes, and refuses a seeding that drops or doubles anyone', async () => {
    ...
    const re = await getBracket(db(), bracket);
    ...
    await expect(setBracketSeeding(db(), bracket, [[pair.side_a, pair.side_a], [lone.side_a, null]])).rejects.toMatchObject({ code: 'bad_seeding' });
    await expect(setBracketSeeding(db(), bracket, [[pair.side_a, pair.side_b], [null, null]])).rejects.toMatchObject({ code: 'bad_seeding' });
    await expect(setBracketSeeding(db(), bracket, [[pair.side_a, pair.side_b]])).rejects.toMatchObject({ code: 'bad_seeding' });
  });
});
...
describe('substitutes and team names', () => {
  ...
  it('swaps a participant everywhere, results included, only for someone on the roster', async () => {
    const id = await seedSolo(['1', '2', '3', '4']);
    const bracket = await addBracket(db(), id, null, NOW);
    const first = (await getBracket(db(), bracket)).filter((m) => m.round === 1)[0];
    ...
    await expect(replaceBracketParticipant(db(), bracket, first.side_a!, 'u:5')).rejects.toMatchObject({ code: 'bad_input' });
    await setSignup(db(), id, '5', 'yes', NOW);
    await replaceBracketParticipant(db(), bracket, first.side_a!, 'u:5');
    ...
    await expect(replaceBracketParticipant(db(), bracket, 'u:nobody', 'u:5')).rejects.toMatchObject({ code: 'missing' });
    await expect(replaceBracketParticipant(db(), bracket, 'u:5', first.side_b!)).rejects.toMatchObject({ code: 'bad_input' });
  });
