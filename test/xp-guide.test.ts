...
describe('the XP guide', () => {
  it('says what a tick pays and how many count', () => {
    ...
    expect(waysLines([kind({ xp: 5, season_cap: 0, auto_source: 'messages', auto_step: 50, claimable: true })])[0]).toBe('• **Helped at an event** — 5 XP per 50 messages, every one counts · the bot gives it');
    ...
    expect(lines).toEqual([
      '• **Helped at an event** — 100 XP, up to 1 per season · you can claim it',
  ...
  it('lists what was collected, over-cap ticks marked', () => {
    ...
    expect(collectedLines([{ ...t, given_at: NOW, event: 'Autumn LAN' }, { ...t, given_at: NOW + 60 }])).toEqual([
      '✅ 7 Sept 2026 · Helped at an event (Autumn LAN) · **100 XP**',
      '▫️ 7 Sept 2026 · Helped at an event · over the cap, 0 XP',
    ]);
  });
  ...
  it('puts it together', () => {
    ...
    const text = xpGuideLines([kind({})], summary, 'https://lahtiag.fi');
    expect(text).toContain('**How to earn XP · season 2026–27**');
    expect(text).toContain('📅 Events attended: 2 · 💬 Discord: 40 messages, 1 h 30 min in voice · counted, and paying XP once');
    expect(text).toContain('**Collected so far: 0 XP**');
