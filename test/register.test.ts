...
const EXTRA_NONE = { discord_id: null, board_note: null, member_type: 'full' as const, honour: null };
const EXTRA_FULL = { discord_id: null, board_note: null, member_type: 'full' as const, honour: null };
...
describe('updateRegisterEntry and status changes', () => {
  it('edits fields, links a Discord id and keeps a board note', async () => {
    ...
    await updateRegisterEntry(
      ...
      { discord_id: '2002', board_note: 'Paid at the door', member_type: 'supporting', honour: null },
      NOW + 5,
    );
    expect(await getRegisterEntry(db(), id)).toMatchObject({
      domicile: 'Hollola',
