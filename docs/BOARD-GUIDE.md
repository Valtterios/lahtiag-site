...
- **Team captains.** Whoever founds a team can add people who signed up
  without a team (the **+** on each name under "Looking for a team"),
...
member on the roster against their own Discord account, 2 add anyone else
from the Discord server, 3 add a walk-in by name. Use one of the first two
for anyone who has an account: the event's Discord role, their ticks and
their stats all follow the signup, and none of that finds a walk-in. Add
Discord user is the friend who came along with a team and never signed up
— the roster marks them a guest, since they are not in the member
register. Both also move someone already on the roster into a team. A
walk-in is a name and nothing else, which is right for a stranger at the
door.*
