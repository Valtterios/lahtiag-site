...
export const POST: APIRoute = async ({ request, redirect }) => {
  ...
  if (!/^[^\s@]+@[^\s@]+$/.test(email) || !email.endsWith(`@${WORKSPACE_DOMAIN}`) || email.length > 120) {
    return redirect('/register?err=bad_email', 303);
  }
  ...
  if (action === 'add') {
    ...
    return redirect('/register?ok=access_added', 303);
  }
  if (action === 'remove') {
    if (email === board.email) return redirect('/register?err=self', 303);
    if (allowedBoardEmails(env.REGISTER_ADMINS).includes(email)) {
      return redirect('/register?err=fixed', 303);
    }
    await removeRegisterAdmin(env.DB, email);
    return redirect('/register?ok=access_removed', 303);
  }
  return redirect('/register?err=bad_email', 303);
};
